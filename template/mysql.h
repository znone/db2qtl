...
inline std::pair<{{query.classname}}::record, bool> {{query.classname}}::get(qtl::{{connection.type}}::database& db, {% for field in primary(query.fieldlist) %} {%if not loop.is_first%},{%endif%}const {{field.type}}& {{field.name}} {%endfor%})
{
	...
	return std::make_pair(rec, db.affected_rows()>0);
}
{% endif %}
{% if query.functions.insert %}
...
{
	uint64_t affected = 0;
	db.execute_direct("INSERT INTO {{escape_token(query.text)}} ({% for field in query.fieldlist %} {%if not loop.is_first%},{%endif%}{{escape_token(field.name)}} {%endfor%}) VALUES ({% for field in query.fieldlist %} {%if not loop.is_first%},{%endif%}? {%endfor%}) "
	 "ON DUPLICATE KEY UPDATE {% for field in not_primary(query.fieldlist) %} {%if not loop.is_first%},{%endif%}{{escape_token(field.name)}}=? {%endfor%}", &affected,
...
{
	uint64_t affected = 0;
	db.execute("DELETE FROM {{escape_token(query.text)}} WHERE {% for field in primary(query.fieldlist) %} {%if not loop.is_first%},{%endif%}{{escape_token(field.name)}}=? {%endfor%}", 0, {% for field in primary(query.fieldlist) %} {%if not loop.is_first%},{%endif%}{{field.name}} {%endfor%}, &affected);
	return affected>0;
}
...
{
	uint64_t affected = 0;
	db.execute_direct("UPDATE {{escape_token(query.text)}} SET {% for field in not_primary(query.fieldlist) %} {%if not loop.is_first%},{%endif%}{{escape_token(field.name)}}=? {%endfor%} WHERE {% for field in primary(query.fieldlist) %} {%if not loop.is_first%},{%endif%}{{field.name}}=? {%endfor%}", &affected,
