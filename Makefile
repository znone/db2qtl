...
PCH_HEADER=stdafx.h
PCH=stdafx.h.gch
OBJ=db2qtl.o mysqlconfig.o qtlconfig.o sqliteconfig.o postgresconfig.o
GENMOD=qtlgen
...
$(GENMOD) : $(GENMOD).o
	$(CC) $(LDFLAGS) -shared -fPIC -o $@ $^ -ldl

%.o : %.cpp $(PCH)
	$(CC) -c $(CFLAGS) $(CXXFLAGS) -o $@ $< 

$(TARGET) : $(OBJ)
	libtool --tag=CXX --mode=link $(CC) $(LDFLAGS) -o $@ $^ -L. -l$(GENMOD) -L$(shell pg_config --libdir)  -lyaml-cpp  $(shell mysql_config --libs)  -lpq -lpgtypes -lsqlite3 -lfmt -lboost_program_options -lboost_filesystem -lboost_system -ldl
